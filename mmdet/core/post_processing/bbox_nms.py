...
def multiclass_nms_with_contour_coefficient(multi_bboxes,
                                            multi_scores,
                                            multi_contours,
                                            multi_coefficients,
                                            score_thr,
                                            ...
                                            score_factors=None):
    num_classes = multi_scores.shape[1]
    bboxes, labels, contours, coefficients = [], [], [], []
    nms_cfg_ = nms_cfg.copy()
    nms_type = nms_cfg_.pop('type', 'nms')
    nms_op = getattr(nms_wrapper, nms_type)
    for i in range(1, num_classes):
        ...
        if multi_bboxes.shape[1] == 4:
            _bboxes = multi_bboxes[cls_inds, :]
            _contours  = multi_contours[cls_inds, :]
        ...
        bboxes.append(cls_dets)
        labels.append(cls_labels)
        contours.append(cls_contours)
        coefficients.append(cls_coefficients)
    if bboxes:
        bboxes = torch.cat(bboxes)
        labels = torch.cat(labels)
        contours = torch.cat(contours)
        ...
        bboxes = multi_bboxes.new_zeros((0, 5))
        labels = multi_bboxes.new_zeros((0, ), dtype=torch.long)
        contours = multi_bboxes.new_zeros((0, 2, 36))
        coefficients = multi_bboxes.new_zeros((0, 16))

    return bboxes, labels, contours, coefficients
